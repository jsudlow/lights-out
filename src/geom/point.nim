...
proc lerp(a, b: int, u: float): int =
  result = a + int(float(b - a) * u)
...
proc lerp*(self, other: ref Point, u: float): ref Point =
  result = newPoint(lerp(self.x, other.x, u),
                    lerp(self.y, other.y, u))
