...
proc flip_neighbors*(self: ref ClickSystem,  i :int, k :int) =
  ...
  if k > 0:
      var north_x = i
      var north_y = k -1
      var active_n = self.scene.grid[((k - 1) * NUM_SQUARES) + i]
      self.scene.grid[((k - 1) * NUM_SQUARES) + i] = not active_n

  #neighbor to the south
  if k < NUM_SQUARES -1:
      var south_x = i
      var south_y = k + 1
      var active_s = self.scene.grid[((k + 1) * NUM_SQUARES) + i]
      self.scene.grid[((k + 1) * NUM_SQUARES) + i] = not active_s

  #neighbor to the west
  if i > 0:
      var west_x = i - 1
      var west_y = k
      var active_w = self.scene.grid[(k * NUM_SQUARES) + i - 1]
      self.scene.grid[(k * NUM_SQUARES) + i - 1] = not active_w

  #neighbor to the east
  if i < NUM_SQUARES -1:
      var east_x = i + 1
      var east_y = k
      var active_e = self.scene.grid[(k * NUM_SQUARES) + i + 1]
      self.scene.grid[(k * NUM_SQUARES) + i + 1] = not active_e
